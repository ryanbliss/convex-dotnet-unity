...
public sealed class ConvexClient : IConvexClient
{
    ...
    public async Task EnsureConnectedAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            // Task.WaitAsync is .NET 6+; netstandard2.1 equivalent via WhenAny
            // against a cancellation-completed task.
            var preConnectCancellation = new TaskCompletionSource<bool>(
                TaskCreationOptions.RunContinuationsAsynchronously);
            using (cancellationToken.Register(
                state => ((TaskCompletionSource<bool>)state!).TrySetCanceled(),
                preConnectCancellation))
            {
                var completedTask = await Task.WhenAny(_preConnectTask, preConnectCancellation.Task)
