...
internal sealed class ConvexWebSocketClient : IDisposable
{
private string deploymentUrl;
private SyncContextCapture? syncContext;
private ReconnectionPolicy? reconnectionPolicy;
private ILogger? logger;
private bool allowInsecureDevelopmentTransport;
public ConvexWebSocketClient(string deploymentUrl, SyncContextCapture? syncContext = null, ReconnectionPolicy? reconnectionPolicy = null, ILogger? logger = null, bool allowInsecureDevelopmentTransport = false)
{
    this._deploymentUrl = deploymentUrl;
    this._allowInsecureDevelopmentTransport = allowInsecureDevelopmentTransport;
    this._reconnectionPolicy = reconnectionPolicy ?? ReconnectionPolicy.Default();
    this._syncContext = syncContext ?? new SyncContextCapture();
    this._logger = logger;
    this.deploymentUrl = deploymentUrl;
    this.syncContext = syncContext;
    this.reconnectionPolicy = reconnectionPolicy;
    this.logger = logger;
    this.allowInsecureDevelopmentTransport = allowInsecureDevelopmentTransport;
}    /// <summary>
    ...
    private readonly bool _allowInsecureDevelopmentTransport ;
    ...
    private async Task ReceiveMessagesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested && _webSocket.State == WebSocketState.Open)
            {
                ...
                if (result.Count > 0)
                {
                    // ArraySegment view keeps the bulk AddRange without a Span
                    // local (not allowed in an async method before C# 13).
